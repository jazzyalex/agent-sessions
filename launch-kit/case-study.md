...
I have never run an ad, bought a placement, or written a launch thread. Every user arrived by word of mouth, a GitHub search, or a link someone dropped in a Slack. Nine and a half months in, the honest numbers are:

- **674 GitHub stars, 44 forks, 0 open issues.**
- **~700 weekly active users.**
- **9,240 total downloads** across releases. When v4.0 shipped on June 28, 2026, it took 294 downloads in its first three days.

The part I find most interesting isn't the totals — it's the shape of the curve. A tool in a fast-moving category is "supposed" to spike and decay. This one did the opposite. Growth averaged around 55 stars a month for the first four months, then *accelerated*: 133 in March 2026, 101 in April, and roughly 80 a month since. Momentum built as the category matured, not as it faded. That told me something about the problem was structural, not novelty-driven.
...
Here's why. Nobody runs one agent CLI anymore. In a single week I'll use Codex for one repo, Claude Code for another, and try something new on a side project. My history is now smeared across nine different ecosystems, each with its own storage, its own schema, and its own idea of what a "session" even is. And this is the part a first-party tool structurally cannot fix: **Codex will never index your Claude sessions.** Claude will never surface your Cursor transcripts. No single vendor has an incentive — or permission — to unify a competitor's data. Cross-agent history, search, and analytics is precisely the job that only a neutral, local, third-party tool can do. The vendors validated the category and then, by their own boundaries, handed me the part they can't touch.

Today Agent Sessions reads nine sources: Codex, Claude Code, Cursor, OpenCode, GitHub Copilot CLI, Hermes, OpenClaw, Antigravity, and Pi. That list is the moat. Every agent a developer adds makes a unified view *more* necessary, not less.

## What I learned that almost nobody else has: nine ecosystems of session data

The genuinely rare expertise I picked up building this isn't UI or Swift. It's that I now understand, in detail, how nine different agent ecosystems persist a conversation to disk — and how little they agree.

...
final class OnboardingContributeCardTests: XCTestCase {
    ...
    /// Both asks due on the same launch: feedback is above contribute in the
    /// chain, so it takes the slot, and the contribute card must not also render.
    /// Once feedback is permanently resolved, the slot falls through.
    @MainActor
    func testFeedbackWinsTheSlotWhenBothAreDue() {
    ...
    func testCardCopyIsFrozen() {
        XCTAssertEqual(ContributeCard.titleText, "Using an agent we don't index?")
        XCTAssertEqual(

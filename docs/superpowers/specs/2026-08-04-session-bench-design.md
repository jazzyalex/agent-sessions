...
> Historical design record. Since 2026-09-09, the canonical inputs, evaluator,
> tests, evidence, generated leaderboard, and corrections live in
> [jazzyalex/session-bench](https://github.com/jazzyalex/session-bench).
> Agent Sessions retains only the temporarily hosted Jekyll view and an exact
> copy of the generated leaderboard.

Date: 2026-08-04. REVISED after external review (Codex, 2026-08-05): scores
are now generated by the standalone repository's `scripts/evaluate.py` from
`data/measurements.json` + `data/verdicts.yml` — the YAML matrix
...
verification results recorded in the generated leaderboard notes).
Authoritative scores live in the standalone repository's `data/leaderboard.yml`; the matrix below
...
1. `docs/bench/` site page (Jekyll) rendered from the vendored
   `docs/_data/session_bench.yml` publication artifact;
   per-gate tooltips citing evidence; version-stamped "bench v1,
   2026-08-04 data".
2. [jazzyalex/session-bench](https://github.com/jazzyalex/session-bench) —
   measurements, verdicts, evaluator, tests, evidence, and generated leaderboard.
...
## v2 candidate gates (from earlier parallel Session-Bench fidelity experiments;
their data was not comparable/importable, but the scenario designs were)

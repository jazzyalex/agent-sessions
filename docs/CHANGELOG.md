...
### Bug Fixes
- **Session-list cards take silence and dismissal seriously.** The app now waits for every active session source to reach ready or error before targeting, freezes one card for the launch, and leaves five quiet days before switching to a different campaign. Star, translation, and add-an-agent invitations are staggered at 25, 40, and 60 distinct opened sessions. Every bounded card advances at most one round per launch, including when its final impression and a soft action occur together; Quota Meter and feedback invitations stop after three ignored launches per release, every temporary exit is labeled “Not now,” and × consistently means the ask should not return. All seven cards share the wrapping layout, preserve their full copy at the narrowest window width, and expose a useful close-button accessibility label. Retention gates count locally hashed session IDs instead of repeated selections of the same session.
...
- **Quota Meter's `Wk` rates no longer turn one short burst or mismatched evidence into a confident weekly rate.** Codex's per-request usage twin is excluded from the cumulative-counter stream instead of re-adding nearly the whole session after every response, completed sessions remain attributable for the full five-minute weekly window, and oversized image/tool records no longer squeeze that history out of a fixed byte tail. Weekly rates wait for one minute of evidence and use a linearly decayed five-minute window instead of replaying the last burst for 150 seconds. Historical and live calibration now share cumulative-counter normalization, require the current weekly anchor, treat the first record after an anchor change as a baseline, preserve event time separately from poll coverage, and reject malformed, incomplete, post-observation, contradictory, or unresolved long-context evidence. Overlapping refreshes keep their poll clock monotonic, and legacy Codex calibration caches are invalidated. Sol requests above 272K input tokens use the published long-context prices, unknown GPT suffixes fail closed unless they are dated snapshots, and the compact rate keeps the plain form `7.1%/h`; its tooltip explains the five-minute estimate.

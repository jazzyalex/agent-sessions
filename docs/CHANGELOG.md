...
- Sessions (Agent Cockpit): Compact mode now includes a dedicated tab/window subtitle column after `Agent`, defaults to a narrower first-run compact width (so session titles truncate earlier), and auto-hides compact header controls when unfocused unless the mouse is hovering the cockpit.

...
- Reorganized both toolbars around how often each control is actually used. The main toolbar keeps source filters, search, Quota Meter, Resume, and the three view toggles; Statistics, Reveal in Finder, Image Browser, Reindex, appearance, and Settings moved into a named overflow menu, and Saved became a scope chip inside the search field. The transcript toolbar is one row instead of two: role filters lead, the Errors chip shows a count when a session has errors, jump arrows appear on the chip being navigated, and view mode moved to the overflow menu with Command-1, Command-2, and Command-3. Session info now uses a gauge icon, and the inspector shares one surface with the rest of the window chrome so light mode no longer shows a white panel beside a warmer transcript. The Delegated row appears only when a transcript actually records delegated work, and a missing Weekly quota states the reason it is missing rather than suggesting a fix that may not apply.

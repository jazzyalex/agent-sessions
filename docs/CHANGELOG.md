...
### Bug Fixes
- **Claude spend was understated by about a sixth in the `$` unit.** Anthropic charges a cache write by how long the cached entry is kept alive: an entry held for five minutes costs 1.25 times the base input rate, one held for an hour costs twice. Agent Sessions charged every Claude cache write at the five-minute rate, and in practice Claude Code almost only writes hour-long entries — across the sixty most recent sessions on the development machine, every one of 182.9 million cache-creation tokens was an hour-long write. Each write is now priced at the rate it was actually billed at, which raises the `$` figure for a typical Claude session by roughly 16.6% and corrects the weekly `%/h` reading that is weighted by it. Agent Sessions also reads which speed a turn was served at, so a session run in Anthropic's fast mode — billed at double the standard Opus rate — prices at its own rates rather than silently at half.
- **The weekly burn-rate figure is no longer thrown off by a countdown.** When Claude's usage reading falls back to reading the terminal, the weekly reset arrives as "in 4h 28m" rather than a date. Every refresh then treated that as a different week and stored a fresh — and wrong — reading each time: the five-hour window's percentage divided by a week's worth of work, about a third below the truth. Any of those could have become the figure the meter used. Stored readings are now keyed only by a reset the provider states as an actual date; the countdown still shows in the meter, which is what it is for.

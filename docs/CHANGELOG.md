...
- **Resuming a Codex guardian session no longer resumes its parent.** Newer Codex builds wrote a guardian subagent's internal session id pointing at the parent session instead of its own, so clicking Resume on a guardian row reopened the parent's session. Guardian rows now resume themselves.

...
- Sessions (Cockpit/Unified): Live session detection now includes Claude and OpenCode in addition to Codex, with mixed-source live rows in Cockpit and source-aware live filtering in Unified Sessions.
...
- Sessions (Claude live-state): Claude probe conflict handling now prefers shell-prompt state over processing recency, and ambiguous non-prompt tails now defer to fallback heuristics to reduce false-active `open` sessions.
...
- Sessions (Cockpit/Claude/OpenCode): Unresolved TTY-backed rows with missing terminal metadata now remain visible (matching focus fallback behavior), while explicit non-iTerm metadata (for example `tmux`) continues to be suppressed as ghost placeholders.

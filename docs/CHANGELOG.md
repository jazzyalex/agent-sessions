...
### Features
- **Experimental Linux console version.** `agent-sessions` is a terminal UI to browse, search, read, copy, and resume local sessions from all supported agents; `agent-sessions-core` is the JSON engine behind it, built from the same Swift parsers and search index as the Mac app, with SQLite linked in. `linux/package.sh arm64|amd64` builds a tarball, `.deb`, and `.rpm`; `scripts/build_unsigned_dmg.sh` builds an unsigned Mac DMG for forks. Nothing is published yet, and the Mac app is unchanged.

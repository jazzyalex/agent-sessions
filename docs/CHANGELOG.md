...
### Features
- **Session-Bench v0.4 adds gate S4, superseded share.** The Signal area now also scores the fraction of stored bytes a final-state-lossless collapse would remove, with a 20% limit, measured on the real corpus and naming its collapse rule — and what the collapse discards — per source. Seeded by external disk measurements ([discussion #54](https://github.com/jazzyalex/agent-sessions/discussions/54), receipt `receipts-2026-08-22-s4.md`): OpenCode fails at 95.8% superseded (a full message snapshot per streaming update; keeping the newest per message turns 378 MB into 16 MB). A rule must hold for an entire source and for the session store itself: Grok tool-call folding is excluded because its prefix property holds for only 66% of tool-call groups, and Codex's unreclaimed SQLite freelist sits in its logs/tracing store rather than the rollout JSONL the bench scores, so Codex scores not_run alongside every other source without a qualifying measured rule. `measure.py` gains freelist and newest-snapshot-per-key extractors; the bench page carries a corrections entry for the rubric change.
- **Devin CLI sessions.** Browsing, search, filtering, Analytics and the transcript view read Devin's shared SQLite store. Devin keeps every retry and edit as a branch, so a session shows the live conversation rather than every path it took to get there, and working directories come from the session record. Settings gains binary and storage-root controls, and Copy Resume Command produces `devin --resume <id>`, falling back to `--continue` — which continues the most recent conversation — only when the session's working directory is known. Devin keeps only active sessions, so there is no archived history to browse, and image extraction is not yet supported. Read against CLI 3000.3.27 with a schema survey of 253 on-disk sessions; re-verified 2026-08-27 on 3000.5.20 and 9-point checklist (`issuecomment-5433779336`), with resume closed via `devin --resume stump-zebu -p` (exit 0).
- **fx (vercel-labs) sessions.** Browsing, search, filtering, Analytics and the transcript view read the per-session checkpoints under `~/.fx/sessions`. Every kind of turn renders: ordinary replies with their narrated tool steps, background commands with where their output went, the summary that replaces auto-compacted history, and interrupted turns with whatever finished before the cut. Text that fx stored as raw bytes rather than UTF-8 decodes back to readable output instead of disappearing. Settings gains binary and storage-root controls, and Copy Resume Command produces `fx --resume <id>`, with workspace-scoped `--continue` as a fallback that is refused when no working directory is known. Image extraction is not yet supported. Read against fx 0.0.4 and re-verified on the installed 0.0.5 against its own format code (9-point checklist `issuecomment-5433779336` on 5 live dirs); `fx session --id --json` succeeds 3/3 non-corrupt and `expect` of `fx --resume <id>` reaches the TTY gate (full interactive reopen needs a TTY), so the flag and session id are valid.

### Bug Fixes
- **Weekly Session Runway reflects recent quota movement.** Its `%/h` now comes from consecutive same-reset provider samples instead of averaging the whole week so far; until a real tick is measured it falls back to `tk/h`. API-equivalent prices were refreshed for GPT-5.6 Sol, Terra and Luna and for Claude Sonnet 5, while the token, `$`, and 5h lenses remain independent.

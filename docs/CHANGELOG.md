...
### Features
- **Session-Bench v0.4 adds gate S4, superseded share.** The Signal area now also scores the fraction of stored bytes a provably lossless collapse would remove, with a 20% limit, measured on the real corpus and naming its collapse rule per source. Seeded by external disk measurements ([discussion #54](https://github.com/jazzyalex/agent-sessions/discussions/54)): OpenCode fails at 95.8% superseded (a full message snapshot per streaming update; keeping the newest per message turns 378 MB into 16 MB) and Codex fails at 90.6% (an SQLite freelist that `auto_vacuum=INCREMENTAL` never reclaims). Only rules proven lossless for an entire source qualify — Grok tool-call folding is excluded because its prefix property holds for only 66% of tool-call groups; sources without a qualifying measured rule score not_run. `measure.py` gains freelist and newest-snapshot-per-key extractors; the bench page carries a corrections entry for the rubric change.

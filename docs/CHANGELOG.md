...
### Bug Fixes
- **Cline custom paths and active transcripts now stay trustworthy.** Invalid session-root drafts no longer replace the working discovery root, `CLINE_DATA_DIR` cannot silently fall back to another profile, a custom CLI binary is considered available only while the successfully probed executable still matches the configured path, and changing that path during a version check cannot publish stale probe state. Missing or partial companion files no longer erase an open transcript, production full loads retain their size bound, and weekly monitoring checks both files' version, identity, and freshness.

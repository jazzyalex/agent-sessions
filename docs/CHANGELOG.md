...
- **Node version managers are found without asking a shell.** Probes read nvm's and fnm's default alias straight from disk, so an rc file that only loads them for a real terminal no longer hides the CLI installed under one.
- **Qwen recovers when a custom binary's probe fails.** A failed probe could leave a "supports nothing" verdict recorded against a path you chose yourself, and unlike auto-detection nothing ever tried that path again — so Copy Resume Command stayed switched off for good. Such a verdict is now dropped when the app starts and the binary is probed again.
- **Codex sessions keep the surface they were indexed with.** Every search-ingest pass wrote over it with nothing, so a Codex session lost its origin moments after being indexed: the list fell back to a `cli` pill for sessions that were Desktop, and a session could change its own label as it finished loading. Origin now travels with the ingest, both writers keep an existing value instead of blanking it, and existing libraries repair themselves once on upgrade.
- **Codex CLI and `codex exec` sessions stop showing as Desktop.** Recent Codex builds stamp "Codex Desktop" as the originator of every session regardless of where it ran, so the real surface has to come from the session's own `source` field — which it now does for CLI and exec runs.
- **Side chats say "side" and inherit their parent's origin.** They were labelled `desk` and always claimed Desktop, whatever the session they branched from actually was.

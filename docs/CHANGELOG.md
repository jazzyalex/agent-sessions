...
- **Resume actions can now open in Ghostty, Kitty, or WezTerm.** Settings → General detects installed supported terminals, keeps one shared default for every resumable CLI agent, and offers Ghostty, Kitty, and WezTerm alongside Terminal, iTerm2, and Warp when available. The existing launch paths for each agent remain in place while the new terminals use shared adapters. Ghostty receives the complete resume command whether already running or fully quit, Kitty reuses an Agent Sessions instance group, and WezTerm opens the command in a new window. Repeated Resume actions are ignored while a launch is pending.

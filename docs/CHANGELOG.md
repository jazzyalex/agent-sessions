...
- **Inline images from a Grok session now appear.** A Grok transcript can carry a real attachment inlined as a data URI, and four separate gates each dropped it silently — so a session with an image showed one in neither the transcript nor the Image Browser. All four are now exhaustive, and the same scanner is extended to Kimi, Pi, Hermes and Cursor, which previously could only ever render a placeholder.

### Bug Fixes
- **An unrelated binary earlier in your PATH no longer masks the real CLI.** Cursor, Kimi, Pi and Grok searched candidate locations group by group and returned the first executable they found, even when it did not advertise the flags the CLI needs — so a same-named binary in the login shell's PATH won before the actual install directory was ever examined, reporting its version as the agent's and refusing resume outright. This one is live rather than theoretical for Grok: Homebrew ships an unrelated `grok` regex utility, while Grok CLI arrives as the `grok-build` cask. Each source now searches one ordered list, and a binary that advertises the right flags always wins.
- **Analytics no longer reports "no sources enabled" over data it has.** Its empty state, the banner warning that totals exclude an agent, and the agent picker were three more hand-written lists, each stopping at a different source. With Grok, Cursor or OpenClaw as your only enabled agent, Analytics claimed nothing was enabled while showing that agent's data, and the picker could not isolate any of the three. All three lists now derive from one source of truth.
- **A Grok session shows the title its own sidecar records.** Grok writes a generated title, but its transcripts open with an injected context preamble rather than your prompt — so once the transcript loaded, the title fell through to that preamble and displayed literal `<user_query>` markup or the opening line of an assistant reply.
- **Grok transcripts open with content in them.** The transcript view keeps one layer per provider and reveals the right one, and Grok had no layer — its indexer was wired up and then never used, which no compiler could catch. A test now checks that every source has one.
- **A Grok session's message count matches its transcript.** The count came from a sidecar field that includes system and reasoning records, both of which render as meta — so a transcript of nothing but meta records claimed three messages and slipped past the filters that hide empty and low-message sessions.

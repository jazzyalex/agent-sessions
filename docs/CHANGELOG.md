...
- **Session-Bench v0.4 adds gate S4, superseded share** — the fraction of stored bytes a final-state-lossless collapse would remove, with a 20% limit, measured on the real corpus. OpenCode fails at 95.8%: keeping only the newest snapshot per message turns 378 MB into 16 MB ([discussion #54](https://github.com/jazzyalex/agent-sessions/discussions/54); full rules and receipts on the [bench page](https://jazzyalex.github.io/agent-sessions/bench/)).

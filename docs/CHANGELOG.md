...
### Bug Fixes
- **Quota Meter's `Wk` rates no longer turn one short burst into a sustained peak.** Codex's per-request usage twin is excluded from the cumulative-counter stream instead of re-adding nearly the whole session after every response, completed sessions remain attributable for the full five-minute weekly window, and oversized image/tool records no longer squeeze that history out of a fixed byte tail. Weekly rates wait for one minute of evidence and use a linearly decayed five-minute window instead of replaying the last burst for 150 seconds. Consecutive live quota ticks are pooled, and a small live span can no longer displace a better-conditioned historical calibration; Sol requests above 272K input tokens use the published long-context prices, unknown GPT suffixes fail closed unless they are dated snapshots, and persisted Codex calibrations carry an activity-accounting revision so legacy payloads are rejected after restart. The compact rate remains `7.1%/h` with no approximation symbol.

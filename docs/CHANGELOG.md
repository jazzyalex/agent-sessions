...
- Sessions (Agent Cockpit): Replaced separate Active/Idle filter pills with a single segmented `Show` control (`All`, `Active`, `Idle`) to remove ambiguous multi-pill states; each segment now includes live counts.

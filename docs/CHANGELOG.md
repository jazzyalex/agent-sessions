...
### Changed
- **The View menu is a single Quota Meter toggle.** The three-mode submenu is replaced by one show/hide item, the standard macOS pattern. **⌘⌥⇧C** now hides the Quota Meter when it is already on screen, where it used to select the mode and bring the window forward.
- **⇧⌘M ("Cycle Cockpit View") is removed.** Cycling has no meaning with one mode, and rebinding a chord that meant "next view" to something else would have inverted it under your fingers.
...
- **Retired with the modes:** the session search field and ⌘K, By Project grouping, the All/Active/Idle filter pills, ⌘1–9 and ⌘0 row focus, the row context menu, the full-mode toolbar row, and the Full/Compact quota footer with its hover drawer. The Quota Meter keeps its provider limit rows, Session Runway drawer, chrome modes, pin, enlarged text, the runway and probe popovers, the open-app and settings buttons, and ⌘W.

...
>
> **W6 outcome (2026-07-02): GATE PASSED — rewrite backstop NOT triggered.** PresenceEngine actor + @Observable presence model + FilteredDefaultsObserver + toolIO skip-gate fix (28f1573b..3de8e692). Idle-with-QM ≤5% (owner-measured, bar <5); window-open idle 5.5% (was 150.7%); AttributeGraph 2,619→61 frames/10s; select-walk 30 steps→2 propagations. Three stacked idle burns, each masked by the one above: presence fan-out → defaults-notification fan-out → 1,115 old sessions re-ingesting every 15 min (skip-gate cohort bug).
>
> **W7 outcome (2026-07-03): List Feel — ACCEPTED by owner** ("far from AgentsView snappiness, but for this iteration let's accept it"). Settled-selection for the transcript pane + HUD scrub-storm kill + row-body diet (77cd42df..39b480b7). Scrub 30 steps: 27 stalls→1, per-step transcript dispatches ~25→0; fast-scroll 120 jumps: 120 stalls→6, HUD storms→0, fallback map off main. NSTableView spike DEFERRED on evidence (Table coordinator = 18/5,228 main-thread samples — the gap was app plumbing, not the rendering layer). Suite 1,170/0; Release green.
>

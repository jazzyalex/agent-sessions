...
enum DeepSeekHarnessFormatError: Error, Equatable, LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        switch self {
        case .unsupportedVersion(let version): return "DeepSeek Harness format v\(version) is unsupported; supported versions are v0 through v3."
        case .invalidHeader: return "DeepSeek Harness session header is invalid."
        case .invalidEnvelope: return "DeepSeek Harness event envelope is invalid."
        case .invalidPayload(let detail): return "DeepSeek Harness event payload is invalid: \(detail)."
        case .invalidReference(let detail): return "DeepSeek Harness event reference is invalid: \(detail)."
        case .unsupportedMigration(let detail): return "DeepSeek Harness migration refuses this log: \(detail)."
        case .sequence(let expected, let actual): return "DeepSeek Harness event sequence expected \(expected), got \(actual)."
        case .unknownRequiredEvent(let type): return "DeepSeek Harness requires unsupported event \(type)."
        case .tornLine(let offset): return "DeepSeek Harness has an incomplete final JSONL record at byte \(offset)."
        case .invalidUTF8(let offset): return "DeepSeek Harness has invalid UTF-8 at byte \(offset)."
        case .invalidJSON(let offset): return "DeepSeek Harness has invalid JSON at byte \(offset)."
        case .corruptFrame(let frame, let offset, let reason): return "DeepSeek Harness Zstandard frame \(frame) is corrupt at byte \(offset): \(reason)."
        case .incompleteFrame(let frame, let offset): return "DeepSeek Harness Zstandard frame \(frame) is incomplete at byte \(offset)."
        case .firstFrameHeaderViolation: return "DeepSeek Harness first Zstandard frame must contain exactly one newline-terminated header record."
        case .encodingMismatch: return "DeepSeek Harness session root mixes plain and Zstandard artifacts."

...
final class CodexDesktopAppLauncher {
    enum LaunchError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .invalidSessionID:
                return String(localized: "A valid Codex session ID is required to continue this session in Codex Desktop.",
                              comment: "Error when a local Codex thread has no valid UUID for a desktop deep link.")
            case .appNotInstalled:
                return String(localized: "Codex Desktop is not installed. Install it, then try continuing this session again.",

...
final class CopilotTerminalLauncher: CopilotTerminalLaunching {
    func launchInTerminal(_ package: CopilotResumeCommandBuilder.CommandPackage) throws {
        ...
        guard process.terminationStatus == 0 else {
            ...
            throw NSError(domain: "CopilotTerminalLauncher", code: Int(process.terminationStatus), userInfo: [NSLocalizedDescriptionKey: err.isEmpty ? "Terminal launch failed." : err])
        }
    }
}
...
final class CopilotITermLauncher: CopilotTerminalLaunching {
    func launchInTerminal(_ package: CopilotResumeCommandBuilder.CommandPackage) throws {
        ...
        guard process.terminationStatus == 0 else {
            let data = stderr.fileHandleForReading.readDataToEndOfFile()
            let err = String(data: data, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            throw NSError(domain: "CopilotITermLauncher", code: Int(process.terminationStatus), userInfo: [NSLocalizedDescriptionKey: err.isEmpty ? "iTerm2 launch failed." : err])
        }
    }
}

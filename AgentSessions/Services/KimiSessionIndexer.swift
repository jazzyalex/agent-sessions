...
final class KimiSessionIndexer: ObservableObject, SessionIndexerProtocol, @unchecked Sendable {
    ...
    @AppStorage(PreferencesKey.Paths.kimiSessionsRootOverride) var sessionsRootOverride: String = ""
    ...
    private var discovery: KimiSessionDiscovery
    ...
    init() {
        let initialOverride = UserDefaults.standard.string(forKey: PreferencesKey.Paths.kimiSessionsRootOverride) ?? ""
        self.discovery = KimiSessionDiscovery(customRoot: initialOverride.isEmpty ? nil : initialOverride)
    ...
    func refresh(mode: IndexRefreshMode = .incremental,
        ...
        if !AgentEnablement.isEnabled(.kimi) { return }

        let currentOverride = UserDefaults.standard.string(forKey: PreferencesKey.Paths.kimiSessionsRootOverride) ?? ""
        if currentOverride != lastOverride {
            discovery = KimiSessionDiscovery(customRoot: currentOverride.isEmpty ? nil : currentOverride)

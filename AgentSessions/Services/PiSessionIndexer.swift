...
final class PiSessionIndexer: ObservableObject, SessionIndexerProtocol, @unchecked Sendable {
    ...
    @AppStorage(PreferencesKey.Paths.piSessionsRootOverride) var sessionsRootOverride: String = ""
    ...
    private var discovery: PiSessionDiscovery
    ...
    init() {
        let initialOverride = UserDefaults.standard.string(forKey: PreferencesKey.Paths.piSessionsRootOverride) ?? ""
        self.discovery = PiSessionDiscovery(customRoot: initialOverride.isEmpty ? nil : initialOverride)
    ...
    func refresh(mode: IndexRefreshMode = .incremental,
        ...
        if !AgentEnablement.isEnabled(.pi) { return }

        let currentOverride = UserDefaults.standard.string(forKey: PreferencesKey.Paths.piSessionsRootOverride) ?? ""
        if currentOverride != lastOverride {
            discovery = PiSessionDiscovery(customRoot: currentOverride.isEmpty ? nil : currentOverride)
        ...
        Task.detached(priority: prio) { [weak self, token, executionProfile] in
            ...
            let config = SessionIndexingEngine.ScanConfig(
                ...
                parseLightweight: { PiSessionParser.parseFile(at: $0) },

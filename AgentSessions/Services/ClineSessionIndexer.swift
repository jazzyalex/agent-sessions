...
final class ClineSessionIndexer: ObservableObject, SessionIndexerProtocol, @unchecked Sendable {
    ...
    @AppStorage(ClinePreferencesKey.sessionsRootOverride) var sessionsRootOverride: String = ""
    ...
    private var discovery: ClineSessionDiscovery
    ...
    init() {
        let initialOverride = UserDefaults.standard.string(forKey: ClinePreferencesKey.sessionsRootOverride) ?? ""
        self.discovery = ClineSessionDiscovery(customRoot: initialOverride.isEmpty ? nil : initialOverride)
    ...
    func refresh(mode: IndexRefreshMode = .incremental,
        ...
        if !AgentEnablement.isEnabled(.cline) { return }

        let currentOverride = UserDefaults.standard.string(forKey: ClinePreferencesKey.sessionsRootOverride) ?? ""
        if currentOverride != lastOverride {
            discovery = ClineSessionDiscovery(customRoot: currentOverride.isEmpty ? nil : currentOverride)
        ...
        Task.detached(priority: prio) { [weak self, token, executionProfile] in
            ...
            let config = SessionIndexingEngine.ScanConfig(
                ...
                parseLightweight: { ClineSessionParser.parseFile(at: $0) },
    ...
    func reloadSession(id: String, force: Bool = false, reason: ReloadReason = .selection) {
        ...
        ioQueue.async {
            ...
            let parsed = ClineSessionParser.parseFileFull(at: url) ?? existing
            self.reloadLock.lock()
    ...
    /// A successful full parse is authoritative for every transcript-derived field.
    /// `??` survives only where neither the manifest nor the messages file it rereads
    /// carries the value at all (timestamps, model). Internal so tests can pin the rule.
    ...
    /// Cline mutates the adjacent messages file as a conversation continues.
    /// Cache invalidation therefore tracks the newest nanosecond timestamp and
    /// combined size of the manifest/transcript pair.
    static func fileStat(for url: URL) -> SessionFileStat? {
        let messagesURL = ClineSessionDiscovery.messagesFile(forManifest: url)
        let values = [url, messagesURL].compactMap {
            try? $0.resourceValues(forKeys: [.contentModificationDateKey, .fileSizeKey])
        }
        guard !values.isEmpty,
              let modified = values.compactMap(\.contentModificationDate).max() else { return nil }
        let size = values.reduce(Int64(0)) { $0 + Int64($1.fileSize ?? 0) }
        let nanoseconds = Int64((modified.timeIntervalSince1970 * 1_000_000_000).rounded())
        return SessionFileStat(mtime: nanoseconds, size: size)
    }
}

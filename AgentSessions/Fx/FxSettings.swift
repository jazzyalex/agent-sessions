...
struct FxProbeRequest: Equatable, Sendable {
    let generation: UInt64
    let binaryOverride: String?
}

enum FxProbeCompletionAcceptance: Equatable {
    case accepted
    case stale(reprobe: FxProbeRequest)
}

/// Preferences backing the fx pane.
///
...
/// the fallback policy is an `FxResumeCoordinator` default.
///
/// `defaultWorkingDirectory` is deliberately absent: fx records each session's
/// workspace in the `workspace_root` field of `session.json`, so resume
/// resolves per session and a user-supplied default would only be a guess that
/// silently redirects `--continue` at an unrelated session.
/// `FxResumeCoordinator` refuses that strategy without a known directory.
@MainActor
final class FxSettings: ObservableObject {
    static let shared = FxSettings()

    enum Keys {
        static let binaryPath = "FxBinaryPath"
        static let resolvedBinaryPath = "FxResolvedBinaryPath"
        static let resolvedSupportsResume = "FxResolvedSupportsResume"
        static let resolvedSupportsContinue = "FxResolvedSupportsContinue"
    }
    ...
    fileprivate init(defaults: UserDefaults = .standard, warmResolvedBinaryCache: Bool = true) {
        ...
        // that does. Two entries are stale: one naming a binary that advertises
        // neither flag (a probe that could not execute the CLI), and one naming a
        // binary that is no longer executable. Both used to be repaired lazily by
        // `validatedCachedResolvedBinaryPath`, which `canCopyResumeCommand`
        // reached on every context-menu render; that call is now the pure
        // `canBuildCopyCommandPlan`. The second case would otherwise survive a
        // relaunch too, since `warmResolvedBinaryPathIfNeeded` only rebuilds an
        // *empty* path. Same fix as `DevinSettings`.
        let cachedAtLaunch = resolvedBinaryPath.trimmingCharacters(in: .whitespacesAndNewlines)
        if !cachedAtLaunch.isEmpty,
           (!resolvedSupportsResume && !resolvedSupportsContinue)
            || !FileManager.default.isExecutableFile(atPath: cachedAtLaunch) {
            clearResolvedBinary()
        }
    ...
    func currentProbeRequest() -> FxProbeRequest {
        FxProbeRequest(
    ...
    func acceptProbeCompletion(
        _ result: Result<FxCLIEnvironment.ProbeResult, FxCLIEnvironment.ProbeError>,
        for request: FxProbeRequest
    ) -> FxProbeCompletionAcceptance {
    ...
    /// advertised capabilities, and fall back to a bare `fx` on PATH when
    /// nothing has been probed yet.
    func copyCommandPlan(sessionID: String) -> (binary: String, strategy: FxResumeCommandBuilder.Strategy)? {
        ...
        return (FxCLIEnvironment.binaryName,
                FxResumeCommandBuilder().strategy(forSessionID: trimmedSessionID))
    }
    ...
    /// row rendered. Same fix, and same reasoning, as `DevinSettings`.
    func canBuildCopyCommandPlan(sessionID: String) -> Bool {
        ...
        if !custom.isEmpty {
            ...
            // Nothing probed yet: `copyCommandPlan` falls back to a bare `fx`
            // and always yields a strategy.
            return true
        }

        guard usableBinary else {
            // The auto branch heals and retries with a bare `fx`; the custom
    ...
    private func validatedCachedResolvedBinaryPath() -> String? {
        ...
        // An fx build that supports neither --resume nor --continue does not
    ...
    private func validatedCustomResolvedBinaryPath(_ custom: String) -> String? {
        ...
        // `fx`, so keeping the dead entry disables resume with no way out.
    ...
    private func capabilityAwarePlan(
        binary: String,
        sessionID: String
    ) -> (binary: String, strategy: FxResumeCommandBuilder.Strategy)? {
...
extension FxSettings {
    static func makeForTesting(defaults: UserDefaults = UserDefaults(suiteName: "FxTests") ?? .standard) -> FxSettings {
        FxSettings(defaults: defaults, warmResolvedBinaryCache: false)
    }
}

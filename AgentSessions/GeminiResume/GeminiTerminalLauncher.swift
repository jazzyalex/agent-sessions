...
final class GeminiTerminalLauncher: GeminiTerminalLaunching {
    func launchInTerminal(_ package: GeminiResumeCommandBuilder.CommandPackage) throws {
        ...
        guard process.terminationStatus == 0 else {
            ...
            throw NSError(domain: "GeminiTerminalLauncher", code: Int(process.terminationStatus), userInfo: [NSLocalizedDescriptionKey: err.isEmpty ? "Terminal launch failed." : err])
        }
    }
}
...
final class GeminiITermLauncher: GeminiTerminalLaunching {
    func launchInTerminal(_ package: GeminiResumeCommandBuilder.CommandPackage) throws {
        ...
        guard process.terminationStatus == 0 else {
            let data = stderr.fileHandleForReading.readDataToEndOfFile()
            let err = String(data: data, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            throw NSError(domain: "GeminiITermLauncher", code: Int(process.terminationStatus), userInfo: [NSLocalizedDescriptionKey: err.isEmpty ? "iTerm2 launch failed." : err])
        }
    }
}

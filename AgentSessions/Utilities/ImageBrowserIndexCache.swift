...
actor ImageBrowserIndexCache {
    ...
    func getOrBuildIndex(for session: Session,
        ...
        case .codex, .claude, .openclaw, .grok, .kimi, .pi, .hermes, .cursor:
            let located: [Base64ImageDataURLScanner.LocatedSpan] = {
                do {
                    ...
                    case .antigravity, .opencode, .copilot, .droid, .qwen, .devin, .fx, .cline, .deepseekHarness:
        ...
        case .droid, .qwen, .devin, .fx, .cline, .deepseekHarness:
            ...
            // and Cursor use once it is built). Cline likewise has no inline
            // image payload. Exhaustive on purpose — this used to be a `default:`, which
